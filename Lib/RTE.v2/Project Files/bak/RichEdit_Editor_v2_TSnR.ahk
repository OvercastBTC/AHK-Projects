...
RTE(){
	...
	; EditW := 800
	; EditH := 1000
	; EditW := (receiver.rect.Get('width'))
	; EditH := (receiver.rect.Get('height'))
	global hMult := 3,
	wMult := 1,
	gHmult := 2,
	w := receiver.rect['width'],
	h := receiver.rect['height'],
	EditW := (w * wMult),
	EditH := (h * hMult),
	GuiW := EditW,
	GuiH := (h * gHmult),
	...
	CurrentLineCount := 0,
	HasFocus := false
	; RTE(hMult,wMult,gHmult,w, h, EditW, EditH, GuiW , GuiH,REW, REH, MarginX, MarginY, GuiTitle, BackColor, FontName, FontSize, FontStyle, TextColor,TextBkColor,WordWrap,AutoURL,Zoom,ShowWysiwyg,CurrentLine,CurrentLineCount,HasFocus){
	; RTE(){
		; --------------------------------------------------------------------------------
		; Create a Gui with RichEdit controls
		; --------------------------------------------------------------------------------
	...
	global MainGui := Gui("+ReSize", GuiTitle)
	; MainGui.OnEvent("Size", MainGuiSize)
	MainGui.OnEvent("Size", GuiReSizer)
	; MainGui.OnEvent("Move", MainGuiSize)
	...
	MainGui.SetFont("s11", "Times New Roman")
	; Options := "xm y+5 w" . EditW . " r20"
	Options := "xm y+5 w" . EditW . " h" . EditH
	Global RE2 := RichEdit(MainGui, Options)
	If !IsObject(RE2){
		Throw("Could not create the RE2 RichEdit control!", -1)
	}
	; RE2.SetOptions(["SELECTIONBAR"])
	RE2.AutoURL(True)
	; RE2.OnEvent('Size', GuiReSizer)
	...
	; MainSB.OnEvent('Size', GuiReSizer)
	MainSB.SetParts(10, 200)
	MainGui.Show()
	CoordMode('Pixel','Window')
	; MainGui.Move(receiver.rect['x'], receiver.rect['y'])
	; DllCall("SetWindowPos", "UInt", pid_RTE, "UInt", 0, "Int", receiver.rect['x'], "Int", receiver.rect['y'], "Int", receiver.rect['width'], "Int", receiver.rect['height'], "UInt", 0x400)
	RE2.Focus()
	UpdateGui()
	Return
	; ======================================================================================================================
	...
	; UpdateGui:
	UpdateGui(*) {
	; UpdateGui(MainFNAME?, MainFSIZE?, MainSB?, RE2?, RE1?,*) {
		FontName := 'Times New Roman', FontCharset := 0, FontStyle := 'N', FontSize := '11', TextColor := 0, TxBkColor := 0
