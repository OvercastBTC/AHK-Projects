...
; REW := 0
REW := w,
; REW := DPI.WinGetClientPos(,,&REW)
; REH := 0,
REH := h,
; REH := DPI.WinGetClientPos(,,,&REH)
MarginX := 10,
MarginY := 10,
; Global 	EditW := 800,
; EditH := 1000,
...
Global hznA := WinActive() = 'ahk_exe hznHorizon.exe'
; Open_File := ''
; --------------------------------------------------------------------------------
;! 								  Menus
; --------------------------------------------------------------------------------
...
RE_MainGui_1()
RE_MainGui_1(*){
	Global FontName, FontSize, MainFNAME, MainFSIZE, MainGui, MainBNSB, MainBNSI, MainBNSU, MainBNSS, MainBNSH, MainBNSL, MainBNSN, MainBNTC, MainBNBC, MainBNCF, MainBNFP, MainBNFM, MainT1, MainBNAL, MainBNAC, MainBNAR, MainBNAJ, MainBN10, MainBN15, MainBN20, MainSB
	GuiNum := 1
	...
	MainBNBC := MainGui.SetFont('s' FontSize, FontName)
	; ---------------------------------------------------------------------------
	; MainFNAME := MainGui.AddEdit("x+10 yp w150 hp +ReadOnly", FontName)
	MainFNAME := MainGui.SetFont('s' FontSize, FontName)
...
UpdateGui(*) {
	...
	; RE_2().MainSB.SetText(Stats.Line . " : " . Stats.LinePos . " (" . Stats.LineCount . ")  [" . Stats.CharCount . "]", 2)
	; _AE_PerMonitor_DPIAwareness()
	MainGuiSize()
	MainGui.Show()
}
...
MainGuiSize(GuiObj?, MinMax?, Width?, Height?) {
	Global GuiW, GuiH, REW, REH, MarginX, MarginY
	Width := 0, Height := 0
	Critical()
	; If (MinMax = 1){
	; 	Return
	...
	If (Width != GuiW || Height != GuiH) {
		...
		; Infos(DPI.WindowToScreen(&REX,&REY,WinA))
		; DPI.ControlGetPos(&REX, &REY, &REW, &REH, GuiObj)
		; REW += Width - MarginX
		; REH += Height + MarginY
		; REW += (Width - MarginX)
		; REH += Height - MarginY
